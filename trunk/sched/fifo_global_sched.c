/*   FILE:
	...
	To compile the program:
	    ...
	    $ gcc -o fifo_global_sched fifo_global_sched.c -lm
...
int start_fifo_main(int argc, char *argv[], int argid)
{
	...
	//sched_event_t *eventLog=NULL;               /* Head of scheduling events list */
	...
	while (1) {
		...
		if (line[0] != '#') {
			...
			new_task.cet = 0.0;
			new_task.e = -1;
			new_task.p = 0; //set_task_edf_priority(&new_task);
			new_task.state = TASK_READY;
			//add task to task list
			t = add_task_list_p_sorted(t, new_task);
			// printf("added task %d =\t%.2f\t%.2f\tdeadline: %.2f\n", new_task.id, new_task.t, new_task.c, new_task.d);
		...
		while (task_in_event) {
			if (((int) (current_time) % (int) (task_in_event -> t)) == 0) {
				...
				while (current_processor) {
					current_processor->tracer = (trace_event *)add_trace_event((trace_event *) current_processor->tracer, new_trace_event);
...
///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		...
		case SCHEDE_FINISH:
			...
			if (current_time > task_in_event -> d) { /* deadline miseed, thus generate event_list */
				...
				while (current_processor) {
					...
					create_trace_list(file_trace, current_processor->tracer, no_task, current_time, (char *) "FIFO");
...
#endif
#ifdef END_ON_MISS_DEADLINE
				printf("Not Scheduled by fifo: %d", 0);
				return 0;
#endif
			}
			...
			task_in_event->e = -1;
			task_in_event->state = (int) TASK_READY;
			task_in_event->p = current_time;
			/* add event_list (task release) for next task activation (since current activation has finished */
			new_event.id = event_id++;
			new_event.t_event = (int) SCHEDE_RELEASE;
			...
			task_to_execute = find_fifo_task_to_execute(t);
			if (task_to_execute) {
				...
				// getchar();
				/* check if the release event_list for this task is in event_list list (i.e., if release time = current_time */
				if (task_to_execute -> r == current_time) {
					// printf("checking for release event_list for task %d in event_list list at time %.2f\n", task_to_execute -> id, current_time);
					new_event.time = current_time;
	...
	while (current_processor) {
		...
		sprintf(file_trace, "%s_p%d.ktr", &basename_trace[0], file_id);
		create_trace_list(file_trace, (trace_event *) current_processor->tracer, no_task, (int) max_time, (char *) "FIFO");
		current_processor = (processor_t *) current_processor->next;
