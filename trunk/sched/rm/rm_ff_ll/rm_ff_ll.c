      ...
      while ( task_not_assigned ) {
         ...
         if (current_processor -> n)
           bound = (current_processor -> n + 1) * (pow(2, (double) 1/(current_processor -> n + 1)) - 1);
            ...
            bound = 1.0;                        /* it is an empty processor */
         // DBG("bound = %.4f\n", bound);
...
processor_t*  start_rm_ff_ll_main(int argc, char *argv[] )
{
   if (argc != 3) {
	...
	return start_rm_ff_ll(atoi(argv[1]), argv[2]);
