/*   FILE:
	...
	To compile the program:
	    ...
	    $ gcc -o rm_global-sched rm_global-sched.c -lm
...
#include "../include/rm_deadlines_global_sched.h"



//#define END_ON_MISS_DEADLINE

int start_rm_deadline_main(int argc, char *argv[], int argid)
{
	...
	while (1) {
		...
		if (line[0] != '#') {
			...
			new_task.deadline = new_task.r + new_task.t;
			new_task.res = NULL;
			new_task.cet = 0.0;
			new_task.e = -1;
			...
			// event_list.time = task.f;            /* start execution at 0 + task phase */
	...
	while (task_in_event) {
		...
		// add task deadline
		new_trace_event.time = (int) current_time + task_in_event-> d;
		...
		case SCHEDE_FINISH:
			...
			if (current_time > task_in_event -> deadline) { /* deadline miseed, thus generate event_list */
				...
				while (current_processor) {
					...
					create_trace_list(file_trace, current_processor->tracer, no_task, current_time, (char *) "RM with deadlines");
			...
			task_in_event ->deadline = task_in_event -> r + task_in_event -> t;
			task_in_event -> cet = 0.0;
			task_in_event -> e = -1;
			task_in_event -> state = (int) TASK_READY;
